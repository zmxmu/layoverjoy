...
class Image {
    ...
    pipInstallFromRequirements(requirementsTxt, options) {
        ...
        if (!fs.statSync(expandedPath).isFile()) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Requirements path ${requirementsTxt} exists but is not a file`);
        }
        const extraArgs = this.formatPipInstallArgs(options);
    ...
    pipInstallFromPyproject(pyprojectToml, options) {
        ...
        if (!fs.statSync(expandedPath).isFile()) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`pyproject.toml path ${pyprojectToml} exists but is not a file`);
        }
        let tomlData;
        try {
            ...
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Invalid pyproject.toml file ${pyprojectToml}: ${errorMessage}`);
        }
        const dependencies = [];
        if (!tomlData || !tomlData.project || !Array.isArray(tomlData.project.dependencies)) {
            ...
            throw new DaytonaError_1.DaytonaInvalidArgumentError(msg);
        }
        ...
        if (options?.optionalDependencies && tomlData.project['optional-dependencies']) {
            ...
            if (typeof optionalGroups !== 'object' || Array.isArray(optionalGroups)) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError('optional-dependencies must be a mapping in pyproject.toml');
            }
    ...
    addLocalFile(localPath, remotePath) {
        ...
        if (!fs.statSync(expandedPath).isFile()) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Local path ${localPath} exists but is not a file`);
        }
        this._contextList.push({ sourcePath: expandedPath, archivePath: expandedPath });
    ...
    addLocalDir(localPath, remotePath) {
        ...
        if (!fs.statSync(expandedPath).isDirectory()) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Local path ${localPath} exists but is not a directory`);
        }
        this._contextList.push({ sourcePath: expandedPath, archivePath: expandedPath });
        this._dockerfile += `COPY ${expandedPath} ${remotePath}\n`;
        return this;
    ...
    env(envVars) {
        ...
        if (nonStringKeys.length) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Image ENV variables must be strings. Invalid keys: ${nonStringKeys}`);
        }
    ...
    entrypoint(entrypointCommands) {
        if (!Array.isArray(entrypointCommands) || !entrypointCommands.every((x) => typeof x === 'string')) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError('entrypoint_commands must be a list of strings');
        }
    ...
    cmd(cmd) {
        if (!Array.isArray(cmd) || !cmd.every((x) => typeof x === 'string')) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError('Image CMD must be a list of strings');
        }
    ...
    dockerfileCommands(dockerfileCommands, contextDir) {
        if (contextDir) {
            ...
            if (!fs.statSync(expandedPath).isDirectory()) {
                throw new DaytonaError_1.DaytonaInvalidArgumentError(`Context path ${contextDir} exists but is not a directory`);
            }
        }
    ...
    static fromDockerfile(path) {
        ...
        if (!fs.statSync(expandedPath).isFile()) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Dockerfile path ${path} exists but is not a file`);
        }
    ...
    flattenStringArgs(functionName, argName, args) {
        ...
        const flatten = (arg) => {
            if (typeof arg === 'string') {
                ...
                throw new DaytonaError_1.DaytonaInvalidArgumentError(`${functionName}: ${argName} must only contain strings`);
            }
        };
        flatten(args);
        return result;
    ...
    static processPythonVersion(pythonVersion) {
        ...
        if (!SUPPORTED_PYTHON_SERIES.includes(pythonVersion)) {
            throw new DaytonaError_1.DaytonaInvalidArgumentError(`Unsupported Python version: ${pythonVersion}. ` +
